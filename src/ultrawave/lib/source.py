...
__all__ = ['PointSource', 'Receiver', 'Shot', 'WaveletSource', 'ToneBurstSource',
...
class WaveletSource(PointSource):
    ...
    __rkwargs__ = PointSource.__rkwargs__ + ['f0', 'a', 'f0', 'cycles']
...
class ToneBurstSource(WaveletSource):
    ...
    def wavelet(self):
        ...
        # creat the tone burst
        dt = self.time_range.step
        tone_length = int(self.cycles/self.f0/dt)
        tone_t = np.arange(0, tone_length) * dt
        tone_burst = np.sin(2*np.pi*self.f0*tone_t)

        # build the Gaussian envelop
        window_x = np.linspace(-3, 3, tone_length)
        mean = 0
        variance = 1
        Gaussian_env = np.exp(-(window_x - mean) ** 2 / (2 * variance))
        tone_burst = tone_burst * Gaussian_env

        src_p = np.zeros(self.time_range.num)
        src_p[:np.size(tone_burst)] = tone_burst

        # scale the source
        # N = 3
        # src_p = src_p * (2 * self.time_range.step / (N * c0 * model.spacing[0]))  #can be modified in operator.py
        return src_p